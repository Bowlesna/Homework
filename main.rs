...
fn main() {
    ...
    //splt xs into useable pieces somce we can't use mutex here
    let p1 = (xs[0]).clone();
    let p2 = (xs[1]).clone();
    ...
    //create one thread per partition and use it to process one partition
    let mut threads = vec![];
    //let mut threads = vec![];
    for i in partitions {
 	let temp1 = i.clone();
	let temp2 = thread::spawn(move || map_data(&temp1));
	threads.push(temp2);
    }

    //collect the intermediate sums from all threads
    let mut intermediate_results : Vec<usize> = Vec::new();
    for i in threads {
	let results = i.join().unwrap(); 
	intermediate_results.push(results);
    }

    //print information abotu the intermediate sums
    println!("Intermediate sums = {:?}", intermediate_results);

    //calls reduce_data to process the intermediate sums
    let sum = reduce_data(&intermediate_sums);

    //prints the final sum computed by reduce data
    println!("Sum = {}", sum);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    //compute size of each vector
    let partition_size = v.len() / num_partitions;
    // Create a vector that will contain vectors of integers
    let mut xs: Vec<Vec<usize>> = Vec::new();
    //check if we need uneven upartitions
    let mut uneven_p = (v[v.len()-1]+1)%(num_partitions);
   
    //initialize bounds
    let mut lower_bound = 0;
    let mut upper_bound;
    //if the modulus is uneven, then include an extra element
    if uneven_p > 0 {
	upper_bound = partition_size+1;
	uneven_p = uneven_p - 1;
    }
    else {
	...
	// Create the dummy variable for vectors of integers
	let mut temp : Vec<usize> = Vec::new();

	//loop to add integers in the input vectors
	...
	//if the modulus is still uneven, add an extra element to this partition
	if uneven_p > 0 {
		upper_bound = upper_bound + partition_size + 1;
		uneven_p = uneven_p - 1;
	}
	else {
	    upper_bound = upper_bound + partition_size;
	}

	//add temp to the vector that will be returned by this function
	xs.push(temp);
    } 
    //return the result vector
    xs
