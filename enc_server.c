...
void encode_message(int connection_socket)  {
	...
	chars_read = recv(connection_socket, &plaintext_length, sizeof(int), 0);
	if (chars_read < 0) {
		error("\nSERVER: ERROR reading plaintext length\n");
	}

	//read messages from socket
	key = read_message(key_length, connection_socket); 
	plaintext = read_message(plaintext_length, connection_socket);

	//encrypt the message
	code = malloc(sizeof(char)*plaintext_length+1);
	memset(code, '\0', plaintext_length+1);
	for (i = 0; i < plaintext_length; i++) {
		temp1 = (((int)(plaintext[i]))-65);
		temp2 = ((int)(key[i])-65);
		if (temp2 == -33) {
			temp2 = 26;
		}
		...
		temp3 = (((temp1 + temp2)%27)+65);
		if (temp3 == 91) {
			temp3 = 32;
		}
		code[i] = (char)(temp3);
	}
	code[plaintext_length] = '\0';

	//send the coded message back to the client
	send_message(code, plaintext_length, connection_socket);
